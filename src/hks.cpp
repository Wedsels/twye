...
// attack speed
        {
            ...
            "            if l1rest[k] == nil or l1rest[k] > (g_FrameCount - l1clock[k]) * 1.3 then\n"
        ...
        },
        {
            ...
            "    if cancelpress and env(ActionCancelRequest, ACTION_ARM_R1) == TRUE then\n"
            "        cancelpress = false\n"
            "    end\n"
            "\n"
            "    local request = GetAttackRequest(is_guard)\n\n    if request == ATTACK_REQUEST_INVALID then\n"
            "        if not cancelpress and b_[\"last\"] ~= nil and string.find(b_[\"last\"], \"W_Attack\") and cancelwait > 0 and IsEnableDualWielding() ~= -1 and env(ActionDuration, ACTION_ARM_R1) > 0 and g_FrameCount - l1time > cancelwait then\n"
            "            cancelpress = true\n"
        ...
        },
        {
            ...
            "        dualtime(l1)\n"
            "        l1time = g_FrameCount\n"
            "        if (not string.find(l1, \"DualLight%d\") or GetDualAttackMaxNumber(HAND_RIGHT) > tonumber(string.match(l1, \"%d\"))) and l1rest[env(GetEquipWeaponCategory, HAND_RIGHT) .. l1] ~= nil then\n"
            "            cancelwait = l1rest[env(GetEquipWeaponCategory, HAND_RIGHT) .. l1] / 1.3\n"
            "        else\n"
            "            cancelwait = 0\n"
            "        end\n"
            "        is_Dual = TRUE\n        ExecEventAllBody(l1)"
